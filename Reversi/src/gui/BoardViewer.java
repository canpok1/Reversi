...
public class BoardViewer {
	...
	/**
	 * カーソルを合わせるマス目を示す横方向のインデックスです。
	 * 描画のための座標ではありません。
	 */
	private int cursorX;

	/**
	 * カーソルを合わせるマス目を示す縦方向のインデックスです。
	 * 描画のための座標ではありません。
	 */
	private int cursorY;
	...
	/**
	 * ボードビューワーを生成します。
	 * @param width 横に並ぶマス目の数
	 * @param height 縦に並ぶマス目の数
	...
	public BoardViewer(int width, int height) {
		...
		for(int y = 0; y < height; y++) {
			for(int x = 0; x < width; x++) {
				this.stones[y][x] = BoardViewer.NOTHING;
			}
		}
	 ...
	 * @param cellY Y座標
	 * @return マス目にある石。
	 * @throws IllegalArgumentException 指定座標にマスがない場合に発生
	 */
	public int getStone(int cellX, int cellY) {

		// 座標が盤面上かをチェック
		if((cellX < 0) || (cellX >= this.boardWidth)
				|| (cellY < 0) || (cellY >= this.boardHeight)) {
	 ...
	 * @param cellY Y座標
	 * @param stone 配置する石
	 * @throws IllegalArgumentException 対応するマスや石がない場合に発生
	 */
	public void setStone(int cellX, int cellY, int stone) {

		// 引数チェック
		if((stone != BoardViewer.NOTHING)
				&& (stone != BoardViewer.WHITE_STONE)
				&& (stone != BoardViewer.BLACK_STONE)) {
	 ...
	 * @param cellX マス目のX座標
	 * @param cellY マス目のY座標
	 * @throws IllegalArgumentException 存在しないマス目の座標を与えた時に発生
	 */
	...
	/**
	 * カーソルの表示状態を設定します。
	 * @param visible カーソルを表示するなら<code>true</code>、表示しないなら<code>false</code>
	 */
	...
	public void draw(PApplet parent) {
		...
		for(int indexY = 0; indexY < this.boardHeight; indexY++) {
			for(int indexX = 0; indexX < this.boardWidth; indexX++) {
				...
				if(this.cursorVisible && (this.cursorX == indexX) && (this.cursorY == indexY)) {
					parent.fill(255, 0, 0);
				} else {
					parent.fill(0, 255, 0);
				}
				...
				parent.rect(drawX, drawY,
						BoardViewer.CELL_SIZE_X, BoardViewer.CELL_SIZE_Y);

				// 石を描画
				parent.strokeWeight(BoardViewer.STONE_EDGE_WIDTH);
				if(this.stones[indexY][indexX] == BoardViewer.BLACK_STONE) {

					parent.fill(0, 0, 0);
					parent.ellipse(drawX, drawY, BoardViewer.CELL_SIZE_X, BoardViewer.CELL_SIZE_Y);

				} else if(this.stones[indexY][indexX] == BoardViewer.WHITE_STONE) {
